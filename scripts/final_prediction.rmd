...
ggplot(final, aes(x = predictions, y = R_pv2p)) +
  ...
  labs(title = "Trump's Predicted v.s. Actual Two-Party Voteshare by State",
       subtitle = "Predictions had an RMSE of 1.969 and Brier Score of 3.876",
       y = "Trump's Actual Voteshare", 
       x = "Trump's Predicted Voteshare") +
  xlim(0, 70) + 
  ylim(0, 70)
...
correct <- read_csv("../data/abbrev.csv") %>%
  ...
  rename(state = State) %>%
  left_join(results, by = "state") %>%
  mutate(correct = ifelse(winner != real_winner, "False", "True"))

correct %>% 
  ggplot(aes(state = state, fill = correct)) +
  geom_statebins() +
  theme_statebins() +
  labs(title = "2020 Presidential Election Prediction Accuracy",
       fill = "",
       caption = "Sources: The New York Times, Gallup, Five-Thirty-Eight") +
