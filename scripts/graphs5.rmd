...
campaign <- read_csv("../data/ad_campaigns_2000-2012.csv")

data <- creative %>%
  left_join(campaign, by = "creative")

ggplot(data, aes(x = cycle.x, fill = ad_purpose)) + 
  geom_bar(position = "dodge")

ggplot(data, aes(x = cycle.x, fill = ad_tone)) + 
  geom_bar(position = "dodge")
...
ads <- read_csv("../data/ads_2020.csv")
...
prob_rep_nj <- predict(nj_rep_glm, newdata = data.frame(avg_poll = 44.5), type ="response")[[1]]
prob_dem_nj <- predict(nj_dem_glm, newdata = data.frame(avg_poll = 50), type ="response")[[1]]
...
hist(sim_dem_nj_2020)
hist(sim_rep_nj_2020)
...
for(current_state in states) {
  ...
  prob_rep <- predict(rep_glm, 
                      newdata = data.frame(avg_poll = 43), 
                      type ="response")[[1]]
  prob_dem <- predict(dem_glm,
                      newdata = data.frame(avg_poll = 52), 
                      type ="response")[[1]]
...
temp <- predictions_df %>%
  ...
  group_by(Code) %>%
  summarise(win_margin = mean(biden_win_margin))
